...
def train_test(model, data, N, num_epochs = 200):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 50 == 0:
            ...
            PATH = os.path.join(DIR_OUTPUT, 'graphSAGE/models/' + nowdate +'-epoch-' + str(epoch))
    ...
    plt.savefig(os.path.join(DIR_OUTPUT,'graphSAGE/test_stats/{}-acc-N-{}.png'.format(nowdate,N)))
    plt.close()
...
logger = LOGGER.getChild('graphSAGE')
...
if __name__ == '__main__':
    ...
    #Make directories
    PATH = os.path.join(DIR_OUTPUT, 'graphSAGE')
    ...
    model = build_SAGE(data, num_layers = 2, hidden_dim = 512)
    ...
    # SAVE
    nowdate = str(datetime.now()).split()[0]
    filename = "graphSAGE/SAGE-classification-{}.json".format(nowdate)

...
def train_test(model, data, N, num_epochs = 200):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 50 == 0:
            ...
            PATH = os.path.join(DIR_OUTPUT, 'AGNN/models/' + nowdate +'-epoch-' + str(epoch))
    ...
    plt.savefig(os.path.join(DIR_OUTPUT,'AGNN/test_stats/{}-acc-N-{}.png'.format(nowdate,N)))
    plt.close()
...
if __name__ == '__main__':
    ...
    #Make directories
    PATH = os.path.join(DIR_OUTPUT, 'AGNN')
    ...
    # SAVE
    nowdate = str(datetime.now()).split()[0]
    filename = "AGNN/AGNN-classification-{}.json".format(nowdate)
